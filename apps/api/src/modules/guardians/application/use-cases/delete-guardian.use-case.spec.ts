...
describe('DeleteGuardianUseCase', () => {
  ...
  it('soft-deletes the guardian successfully', async () => {
    const guardian = makeGuardian();
    guardianRepo.findById.mockResolvedValue(guardian);
    guardianRepo.softDelete.mockResolvedValue(undefined);

    await useCase.execute({ guardianId: 'guardian-1', organizationId: 'org-1' });

    expect(guardianRepo.findById).toHaveBeenCalledWith('guardian-1', 'org-1');
    expect(guardianRepo.softDelete).toHaveBeenCalledWith('guardian-1', 'org-1');
  });
  ...
  it('does not call softDelete when guardian is not found', async () => {
    guardianRepo.findById.mockResolvedValue(null);
    ...
    expect(guardianRepo.softDelete).not.toHaveBeenCalled();
  });
});
